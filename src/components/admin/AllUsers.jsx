...
const AllUsers = () => {
    ...
    const handleSaveUser = async () => {
        try {
            ...
            await axios.put(`${import.meta.env.VITE_API_URL}/api/v1/users/${editUserId}`, userData);
    ...
    const handlePageChange = (newPage) => {
        if (newPage >= 1 && newPage <= Math.ceil(users.length / pagination.pageSize)) {
            setPagination(prev => ({
                ...prev,
                page: newPage,
            }));
                ...
                <div>
                            ...
                            <table className="min-w-full border-collapse border border-gray-300">
                                ...
                                <tbody>
                                    {paginatedUsers.map((user) => (
                                        <tr key={user._id} className="hover:bg-gray-100 hover:text-black cursor-pointer">
                                            <td className="border border-gray-300 p-2" onClick={() => handleonClick(user)}>View</td>
                                            ...
                                            <td className="border border-gray-300 text-center p-2">
                                                {editUserId === user._id ? (
                                                    <button className="text-green-500" onClick={handleSaveUser}>
                                                        <Save />
                                                    </button>
                                                ) : (
                                                    <>
                                                        <button className="text-yellow-500" onClick={() => handleEditUser(user)}>
                                                            <Edit />
                                                        </button>
                                                    </>
                                                )}
                    ...
                    <div className="mt-4 flex justify-center space-x-2">
                        ...
                        <span className="my-auto text-white">
                            Page {pagination.page} of {Math.ceil(users.length / pagination.pageSize)}
                        </span>
                        <button
                            className="px-4 py-2 bg-gray-300 rounded"
                            onClick={() => handlePageChange(pagination.page + 1)}
