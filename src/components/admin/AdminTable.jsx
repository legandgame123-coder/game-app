...
const AdminTable = () => {
    ...
    const fetchAdmins = async () => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/admin/get-admin`);
    ...
    const handleSave = async () => {
        try {
            await axios.put(`${import.meta.env.VITE_API_URL}/api/v1/admin/update-admin`, editedAdmin);
            setEditIndex(null);
            fetchAdmins();
    ...
    const handleDelete = async (email) => {
        ...
        try {
            await axios.delete(`${import.meta.env.VITE_API_URL}/api/v1/admin/delete-admin`, {
                headers: {
                                ...
                                <td className="p-2">
                                    {editIndex === index ? (
                                        <input
                                            ...
                                            onChange={(e) =>
                                                setEditedAdmin({ ...editedAdmin, fullName: e.target.value })
                                            }
                                    ...
                                    {editIndex === index ? (
                                        <div className="flex flex-wrap gap-2">
                                            {ACCESS_OPTIONS.map((page) => {
                                                const isActive = editedAdmin.accessPages?.includes(page);
                                                return (
                                                    <button
                                                        key={page}
                                                        type="button"
                                                        ...
                                                        onClick={() => {
                                                            const currentAccess = editedAdmin.accessPages || [];
                                                            const updatedAccess = isActive
                                                                ? currentAccess.filter((p) => p !== page)
                                                                : [...currentAccess, page];

                                                            setEditedAdmin({ ...editedAdmin, accessPages: updatedAccess });
                                                        }}
                                                    >
                                                        {page}
                                                    </button>
